...
Page({
  ...
  inputValueChange(e){
      if(!e.detail.value){
        this.initArea(this.data.options)
        return
      }
    //中文拼接一起能够实现中英文匹配
    let merge = this.data.options.map(item => {
      return {
    ...
    this.data.options.forEach((item,i)=>{
         if(merge[i].value.match(e.detail.value)!=null){
           result.push(item)
         }
    })
    this.initArea(result)
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  ...
  tohome(data){
  wx.setStorageSync('nation', data.target.dataset.item) 
      wx.switchTab({
        url: `/pages/home/home`,
      })
  },
